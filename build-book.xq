...
"
   ...
   case element(orderedlist) return for $item in $element/listitem return "
" || string-join(((1 to ($level - 1)) ! "    "), "") || "* " || normalize-space(string-join(for $child in $item/node() return local:docbook-to-markdown($child, $level + 1), ""))
   case element(itemizedlist) return for $item at $i in $element/listitem return "
" || string-join(((1 to ($level - 1)) ! "    "), "") || $i || ". " || normalize-space(string-join(for $child in $item/node() return local:docbook-to-markdown($child, $level + 1), ""))
